...
<body>
  <div class="page">
    <div class="nav">
      <a href="../index.html">ΓåÉ Archive index</a>
        ...
        for cid in children_ids:
            ...
            if child:
                items.append(
                    ...
                    f'<span class="muted">ΓÇö {html.escape(str(child.get("author") or "(unknown)"))}</span></li>'
                )
            else:
    ...
    def make_list(items: List[str], empty: str) -> str:
        ...
        for cid in items:
            ...
            parts.append(
                ...
                f"<span class='muted'>ΓÇö {html.escape(str(node.get('author') or '(unknown)'))}</span></li>"
            )
...
function showDetails(n) {{
  ...
  const openLink = n.artifact_href
    ? `<a class="open-link" href="${n.artifact_href}" target="_blank" rel="noopener">Open artifact page ΓåÆ</a>`
    : "";
